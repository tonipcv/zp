...
export default function Login() {
  ...
  // Step 1: Submit email and password, then send verification code
  const handleEmailPasswordSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!checkUserResponse.ok) {
        throw new Error(checkUserData.error || 'User not found');
      }

      // Verify password (new endpoint needed)
      ...
      const passwordData = await verifyPasswordResponse.json();

      if (!verifyPasswordResponse.ok) {
        throw new Error(passwordData.error || 'Invalid password');
      }
      ...
      console.error('Error in login step 1:', err);
  ...
  // Step 2: Submit verification code and complete login
  const handleVerifyCodeAndLogin = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setIsSubmitting(true);
    setError(null);
    ...
    <div className="min-h-screen bg-[#1c1d20] font-normal tracking-[-0.01em]">
      <div className="min-h-screen flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-[380px] bg-[#1c1d20] p-8">
          ...
          {step === 2 && (
            <form onSubmit={handleVerifyCodeAndLogin} className="space-y-6" autoComplete="off">
              <div>
